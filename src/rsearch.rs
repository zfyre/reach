...
use clap::Parser;
use regex::Regex;
use crate::{apis::gemini_query, config::{ApiConfig, ApiKeys}, display::RawOuts, errors::ReachError};
...
impl Context {
    ...
    async fn add(&mut self, url: &[String], query: &str) -> Result<(), ReachError> {
        ...
        let urls_md: Vec<_> = url_to_md.iter()
...
async fn get_markdown(url: &str) -> Result<String, ReachError> {
